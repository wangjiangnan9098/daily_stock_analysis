...
from src.llm import BrowserCliAdapter
...
5. **风险优先级**：舆情中的风险点要醒目标出"""
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self._use_browser_cli = False  # 是否使用 browser-auth CLI
        self._openai_client = None  # OpenAI 客户端
        self._anthropic_client = None  # Anthropic 客户端
        self._browser_cli_adapter: Optional[BrowserCliAdapter] = None
        self._allow_apikey_fallback = bool(config.llm_browser_cli_allow_apikey_fallback)
        self._auth_mode = (config.llm_auth_mode or "api_key").strip().lower()

        if self._auth_mode == "browser_cli":
            self._init_browser_cli_adapter()
            if self._use_browser_cli and not self._allow_apikey_fallback:
                return
            if not self._use_browser_cli and not self._allow_apikey_fallback:
                logger.warning(
                    "LLM_AUTH_MODE=browser_cli but CLI is unavailable and API key fallback is disabled"
                )
                return
        ...
        if not self._model and not self._anthropic_client and not self._openai_client:
            if not self._use_browser_cli:
                logger.warning("No AI API Key configured, AI analysis will be unavailable")

    def _init_browser_cli_adapter(self) -> None:
        """Initialize browser-auth CLI adapter."""
        config = get_config()
        model_name = (
            (config.llm_browser_cli_model or "").strip()
            or (config.openai_model or "").strip()
            or "gpt-4o-mini"
        )
        try:
            adapter = BrowserCliAdapter(
                command=config.llm_browser_cli_command,
                model_name=model_name,
                timeout_sec=config.llm_browser_cli_timeout_sec,
                login_command=config.llm_browser_cli_login_command,
                auth_check_command=config.llm_browser_cli_auth_check_command,
                auto_login=config.llm_browser_cli_auto_login,
            )
            if not adapter.is_available():
                logger.warning(
                    "Browser CLI command is not available: %s",
                    config.llm_browser_cli_command,
                )
                return
            if config.llm_browser_cli_auto_login:
                try:
                    adapter.ensure_authenticated()
                except Exception as auth_error:
                    logger.warning("Browser CLI auth check/login failed: %s", auth_error)
                    if not self._allow_apikey_fallback:
                        return
            self._browser_cli_adapter = adapter
            self._use_browser_cli = True
            self._current_model_name = model_name
            logger.info(
                "Browser CLI auth initialized (command: %s, model: %s)",
                config.llm_browser_cli_command,
                model_name,
            )
        except Exception as e:
            logger.error("Browser CLI auth initialization failed: %s", e)
    ...
    def is_available(self) -> bool:
        """检查分析器是否可用。"""
        return (
            (self._use_browser_cli and self._browser_cli_adapter is not None)
            or
    ...
    def _call_browser_cli(self, prompt: str, generation_config: dict) -> str:
        """Call browser-auth CLI adapter with retries."""
        if not self._browser_cli_adapter:
            raise RuntimeError("Browser CLI adapter is not initialized")
        ...
        for attempt in range(max_retries):
            try:
                if attempt > 0:
                    ...
                    delay = min(delay, 60)
                    logger.info("[BrowserCLI] retry %s/%s, waiting %.1fs...", attempt + 1, max_retries, delay)
                    time.sleep(delay)

                return self._browser_cli_adapter.generate_text(
                    prompt=prompt,
                ...
                if attempt == max_retries - 1:
                    raise RuntimeError(f"Browser CLI call failed after retries: {error_str}") from e
                logger.warning(
                    "[BrowserCLI] call failed, attempt %s/%s: %s",
                    attempt + 1,
                    max_retries,
                    error_str[:120],
                )
    ...
    def _call_api_with_retry(self, prompt: str, generation_config: dict) -> str:
        ...
        Returns:
            响应文本
        """
        if self._use_browser_cli and self._browser_cli_adapter:
            try:
                return self._call_browser_cli(prompt, generation_config)
            except Exception as cli_error:
                if not self._allow_apikey_fallback:
                    raise cli_error
                if not (self._use_anthropic or self._use_openai or self._model):
                    raise cli_error
                logger.warning(
                    "[BrowserCLI] call failed, falling back to API key providers: %s",
                    cli_error,
                )
    ...
    def analyze(
        ...
        if not self.is_available():
            return AnalysisResult(
                ...
                operation_advice='持有',
                confidence_level='低',
                analysis_summary='AI analysis is unavailable (API key or browser CLI is not ready)',
                risk_warning='Please configure API keys or browser CLI auth and retry',
                success=False,
                error_message='LLM auth not configured',
            )
        ...
        try:
            ...
            # 记录实际使用的 API 提供方
            api_provider = (
                "BrowserCLI" if self._use_browser_cli
                else "OpenAI" if self._use_openai
