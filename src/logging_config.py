...
def setup_logging(
    ...
    # Move misplaced log files from current directory into configured log folder.
    _relocate_if_exists(Path(f"{log_prefix}_{today_str}.log"), log_file)
    _relocate_if_exists(Path(f"{log_prefix}_debug_{today_str}.log"), debug_log_file)
...
def _relocate_if_exists(src: Path, dst: Path) -> None:
    """Relocate a log file to the target directory if it was created in CWD."""
    if not src.exists():
        return
    try:
    ...
    if dst.exists():
        try:
            with src.open("r", encoding="utf-8", errors="ignore") as sf, dst.open("a", encoding="utf-8") as df:
                content = sf.read()
                if content:
