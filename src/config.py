...
class Config:
    ...
    llm_auth_mode: str = "api_key"  # api_key | browser_cli
    llm_browser_cli_command: str = "codex"
    llm_browser_cli_timeout_sec: int = 120
    llm_browser_cli_model: Optional[str] = None
    llm_browser_cli_allow_apikey_fallback: bool = False
    llm_browser_cli_auto_login: bool = True
    llm_browser_cli_login_command: str = "codex login"
    llm_browser_cli_auth_check_command: str = "codex login status"
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        default_wechat_max_bytes = 2048 if wechat_msg_type_lower == 'text' else 4000
        wechat_max_bytes = cls._env_int('WECHAT_MAX_BYTES', default_wechat_max_bytes)
        ...
        return cls(
            ...
            gemini_temperature=cls._env_float('GEMINI_TEMPERATURE', 0.7),
            gemini_request_delay=cls._env_float('GEMINI_REQUEST_DELAY', 2.0),
            gemini_max_retries=cls._env_int('GEMINI_MAX_RETRIES', 5),
            gemini_retry_delay=cls._env_float('GEMINI_RETRY_DELAY', 5.0),
            anthropic_api_key=os.getenv('ANTHROPIC_API_KEY'),
            anthropic_model=os.getenv('ANTHROPIC_MODEL', 'claude-3-5-sonnet-20241022'),
            anthropic_temperature=cls._env_float('ANTHROPIC_TEMPERATURE', 0.7),
            anthropic_max_tokens=cls._env_int('ANTHROPIC_MAX_TOKENS', 8192),
            openai_api_key=os.getenv('OPENAI_API_KEY'),
            openai_base_url=os.getenv('OPENAI_BASE_URL'),
            openai_model=os.getenv('OPENAI_MODEL', 'gpt-4o-mini'),
            openai_vision_model=os.getenv('OPENAI_VISION_MODEL') or None,
            openai_temperature=cls._env_float('OPENAI_TEMPERATURE', 0.7),
            llm_auth_mode=cls._parse_llm_auth_mode(os.getenv('LLM_AUTH_MODE', 'api_key')),
            llm_browser_cli_command=(os.getenv('LLM_BROWSER_CLI_COMMAND', 'codex') or 'codex').strip(),
            llm_browser_cli_timeout_sec=max(10, cls._env_int('LLM_BROWSER_CLI_TIMEOUT_SEC', 120)),
            llm_browser_cli_model=(os.getenv('LLM_BROWSER_CLI_MODEL') or '').strip() or None,
            llm_browser_cli_allow_apikey_fallback=cls._env_bool(
                'LLM_BROWSER_CLI_ALLOW_APIKEY_FALLBACK', False
            ),
            llm_browser_cli_auto_login=cls._env_bool('LLM_BROWSER_CLI_AUTO_LOGIN', True),
            llm_browser_cli_login_command=(
                os.getenv('LLM_BROWSER_CLI_LOGIN_COMMAND', 'codex login') or 'codex login'
            ).strip(),
            llm_browser_cli_auth_check_command=(
                os.getenv('LLM_BROWSER_CLI_AUTH_CHECK_COMMAND', 'codex login status') or 'codex login status'
            ).strip(),
            bocha_api_keys=bocha_api_keys,
            tavily_api_keys=tavily_api_keys,
            brave_api_keys=brave_api_keys,
            serpapi_keys=serpapi_keys,
            news_max_age_days=max(1, cls._env_int('NEWS_MAX_AGE_DAYS', 3)),
            bias_threshold=max(1.0, cls._env_float('BIAS_THRESHOLD', 5.0)),
            ...
            analysis_delay=cls._env_float('ANALYSIS_DELAY', 0.0),
            merge_email_notification=os.getenv('MERGE_EMAIL_NOTIFICATION', 'false').lower() == 'true',
            feishu_max_bytes=cls._env_int('FEISHU_MAX_BYTES', 20000),
            ...
            markdown_to_image_max_chars=cls._env_int('MARKDOWN_TO_IMAGE_MAX_CHARS', 15000),
            database_path=cls._env_str('DATABASE_PATH', './data/stock_analysis.db'),
            save_context_snapshot=os.getenv('SAVE_CONTEXT_SNAPSHOT', 'true').lower() == 'true',
            backtest_enabled=os.getenv('BACKTEST_ENABLED', 'true').lower() == 'true',
            backtest_eval_window_days=cls._env_int('BACKTEST_EVAL_WINDOW_DAYS', 10),
            backtest_min_age_days=cls._env_int('BACKTEST_MIN_AGE_DAYS', 14),
            backtest_engine_version=os.getenv('BACKTEST_ENGINE_VERSION', 'v1'),
            backtest_neutral_band_pct=cls._env_float('BACKTEST_NEUTRAL_BAND_PCT', 2.0),
            log_dir=cls._env_str('LOG_DIR', './logs'),
            log_level=cls._env_str('LOG_LEVEL', 'INFO'),
            max_workers=cls._env_int('MAX_WORKERS', 3),
            ...
            webui_port=cls._env_int('WEBUI_PORT', 8000),
            # 机器人配置
            bot_enabled=os.getenv('BOT_ENABLED', 'true').lower() == 'true',
            bot_command_prefix=os.getenv('BOT_COMMAND_PREFIX', '/'),
            bot_rate_limit_requests=cls._env_int('BOT_RATE_LIMIT_REQUESTS', 10),
            bot_rate_limit_window=cls._env_int('BOT_RATE_LIMIT_WINDOW', 60),
            ...
            realtime_cache_ttl=cls._env_int('REALTIME_CACHE_TTL', 600),
            circuit_breaker_cooldown=cls._env_int('CIRCUIT_BREAKER_COOLDOWN', 300)
        )
    ...
    @classmethod
    def _parse_llm_auth_mode(cls, value: str) -> str:
        """Parse llm auth mode and fallback to api_key for invalid values."""
        import logging

        mode = (value or 'api_key').strip().lower()
        if mode in ('api_key', 'browser_cli'):
            return mode
        logging.getLogger(__name__).warning(
            "LLM_AUTH_MODE value '%s' is invalid, fallback to 'api_key' (allowed: api_key/browser_cli)",
            value,
        )
        return 'api_key'
    ...
    def _env_int(cls, key: str, default: int) -> int:
        """Read integer env with empty/invalid fallback."""
        raw = os.getenv(key)
        if raw is None:
            return default
        raw = raw.strip()
        ...
        try:
            ...
            logging.getLogger(__name__).warning(
                "Invalid integer env %s=%r, fallback to %s", key, raw, default
            )
            return default

    @classmethod
    def _env_float(cls, key: str, default: float) -> float:
        """Read float env with empty/invalid fallback."""
        raw = os.getenv(key)
        if raw is None:
            return default
        raw = raw.strip()
        ...
        try:
            ...
            logging.getLogger(__name__).warning(
                "Invalid float env %s=%r, fallback to %s", key, raw, default
            )
            return default

    @classmethod
    def _env_str(cls, key: str, default: str) -> str:
        """Read string env with empty fallback."""
        raw = os.getenv(key)
        if raw is None:
            return default
        value = raw.strip()
    ...
    def _env_bool(cls, key: str, default: bool) -> bool:
        """Read boolean env with empty/invalid fallback."""
        raw = os.getenv(key)
        if raw is None:
            return default
        ...
        if value in {"0", "false", "no", "off"}:
            return False
        logging.getLogger(__name__).warning(
            "Invalid boolean env %s=%r, fallback to %s", key, raw, default
        )
        return default
    ...
    def validate(self) -> List[str]:
        ...
        apikey_required = self.llm_auth_mode != "browser_cli" or self.llm_browser_cli_allow_apikey_fallback
        if apikey_required:
        ...
        if self.llm_auth_mode == "browser_cli":
            if not self.llm_browser_cli_command:
                warnings.append("警告：LLM_AUTH_MODE=browser_cli 但未配置 LLM_BROWSER_CLI_COMMAND")
            if self.llm_browser_cli_auto_login and not self.llm_browser_cli_login_command:
                warnings.append("警告：已开启 LLM_BROWSER_CLI_AUTO_LOGIN 但未配置 LLM_BROWSER_CLI_LOGIN_COMMAND")
            if self.llm_browser_cli_timeout_sec < 10:
                warnings.append("提示：LLM_BROWSER_CLI_TIMEOUT_SEC 建议不低于 10 秒")
            if self.llm_browser_cli_allow_apikey_fallback:
                warnings.append("提示：browser_cli 已启用 API Key 回退，可能增加 API 额度消耗")
