...
def _select_vision_provider() -> str:
    """Return preferred vision provider: browser_cli / gemini / anthropic / openai."""
    cfg = get_config()
    if (cfg.llm_auth_mode or "").strip().lower() == "browser_cli":
        return "browser_cli"
...
def _build_browser_cli_adapter(cfg) -> Optional[BrowserCliAdapter]:
    """Build browser CLI adapter from runtime config."""
    model_name = (
        (cfg.llm_browser_cli_model or "").strip()
        or (cfg.openai_vision_model or "").strip()
        or (cfg.openai_model or "").strip()
        or "gpt-4o-mini"
    )
    adapter = BrowserCliAdapter(
        command=cfg.llm_browser_cli_command,
        model_name=model_name,
        timeout_sec=cfg.llm_browser_cli_timeout_sec,
        login_command=cfg.llm_browser_cli_login_command,
        auth_check_command=cfg.llm_browser_cli_auth_check_command,
        auto_login=cfg.llm_browser_cli_auto_login,
    )
    if not adapter.is_available():
        return None
    if cfg.llm_browser_cli_auto_login:
        try:
            adapter.ensure_authenticated()
        except Exception as e:
            logger.warning("[ImageExtractor] browser_cli auth check/login failed: %s", e)
            return None
    return adapter


def _call_browser_cli_vision(
    image_b64: str,
    mime_type: str,
    adapter: BrowserCliAdapter,
) -> str:
    """Call browser-auth CLI for vision extraction."""
    return adapter.generate_vision(
        image_b64=image_b64,
        mime_type=mime_type,
...
def extract_stock_codes_from_image(
    ...
    if not provider:
        raise ValueError(
            "未配置 Vision API。请设置 browser_cli 或 GEMINI_API_KEY、ANTHROPIC_API_KEY、OPENAI_API_KEY。"
        )

    image_b64 = base64.b64encode(image_bytes).decode("ascii")

    allow_fallback = bool(cfg.llm_browser_cli_allow_apikey_fallback)
    if provider == "browser_cli":
        order = ["browser_cli"]
        if allow_fallback:
            order.extend(["gemini", "anthropic", "openai"])
    else:
        order = [provider]
    ...
    for p in order:
        ...
        if p == "browser_cli":
            browser_adapter = _build_browser_cli_adapter(cfg)
            if not browser_adapter:
                last_err = ValueError(f"Browser CLI command is unavailable: {cfg.llm_browser_cli_command}")
                continue
        ...
        try:
            if p == "browser_cli":
                raw = _call_browser_cli_vision(image_b64, mime_type, browser_adapter)
            elif p == "gemini":
    ...
    raise ValueError(
        "所有 Vision API 调用均失败，请检查 browser_cli 登录态、API Key 与网络。"
    ) from last_err
