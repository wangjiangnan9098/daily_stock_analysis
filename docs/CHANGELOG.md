...
### 新增
- 🔐 **Browser CLI 模型认证（GPT 优先）**
  - 新增 `LLM_AUTH_MODE=browser_cli`，可通过 browser-auth CLI 会话调用模型
  - 新增配置：`LLM_BROWSER_CLI_COMMAND`、`LLM_BROWSER_CLI_MODEL`、`LLM_BROWSER_CLI_TIMEOUT_SEC`、`LLM_BROWSER_CLI_AUTO_LOGIN`、`LLM_BROWSER_CLI_LOGIN_COMMAND`、`LLM_BROWSER_CLI_AUTH_CHECK_COMMAND`、`LLM_BROWSER_CLI_ALLOW_APIKEY_FALLBACK`
  - 新增 `python main.py --llm-browser-login`，可先独立完成登录授权，再运行分析任务
  - 文本分析与图片识别链路均支持 browser_cli；当开启回退开关时可降级到 API Key 供应商
- **大盘复盘可选区域** (Issue #299)
