...
const fieldTitleMap: Record<string, string> = {
  ...
  LLM_AUTH_MODE: '模型认证模式',
  LLM_BROWSER_CLI_COMMAND: 'Browser CLI 命令',
  LLM_BROWSER_CLI_MODEL: 'Browser CLI 模型',
  LLM_BROWSER_CLI_TIMEOUT_SEC: 'Browser CLI 超时(秒)',
  LLM_BROWSER_CLI_AUTO_LOGIN: 'Browser CLI 自动登录',
  LLM_BROWSER_CLI_LOGIN_COMMAND: 'Browser CLI 登录命令',
  LLM_BROWSER_CLI_AUTH_CHECK_COMMAND: 'Browser CLI 登录态检查命令',
  LLM_BROWSER_CLI_ALLOW_APIKEY_FALLBACK: '允许 API Key 回退',
...
const fieldDescriptionMap: Record<string, string> = {
  ...
  LLM_AUTH_MODE: '选择模型认证方式：api_key 或 browser_cli。',
  LLM_BROWSER_CLI_COMMAND: 'browser_cli 模式下调用的 CLI 命令，例如 codex。',
  LLM_BROWSER_CLI_MODEL: 'browser_cli 模式优先模型；留空时复用 OPENAI_MODEL。',
  LLM_BROWSER_CLI_TIMEOUT_SEC: 'browser_cli 单次调用超时时间（秒）。',
  LLM_BROWSER_CLI_AUTO_LOGIN: '未登录时是否自动执行登录命令。',
  LLM_BROWSER_CLI_LOGIN_COMMAND: 'browser_cli 自动登录命令，例如 codex login。',
  LLM_BROWSER_CLI_AUTH_CHECK_COMMAND: 'browser_cli 登录态检查命令，例如 codex login status。',
  LLM_BROWSER_CLI_ALLOW_APIKEY_FALLBACK: 'browser_cli 失败时是否允许回退到 API Key 供应商。',
