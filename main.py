...
示例:
  ...
  python main.py --llm-browser-login  # 仅执行 browser auth 登录
        '''
    )
    ...
    parser.add_argument(
        '--llm-browser-login',
        action='store_true',
        help='仅执行 browser CLI 登录检查与授权，不运行分析任务'
    )

    return parser.parse_args()


def run_llm_browser_login(config: Config) -> int:
    """Run browser CLI authentication flow only."""
    ...
    adapter = BrowserCliAdapter(
        ...
        timeout_sec=max(config.llm_browser_cli_timeout_sec, 120),
        login_command=config.llm_browser_cli_login_command,
        auth_check_command=config.llm_browser_cli_auth_check_command,
        auto_login=True,
    )
    if not adapter.is_available():
        logger.error("Browser CLI command is unavailable: %s", config.llm_browser_cli_command)
        logger.error(
            "请确认命令已安装且可执行，或在 .env 中将 LLM_BROWSER_CLI_COMMAND 设置为绝对路径（例如 /opt/homebrew/bin/codex）"
        )
        return 1

    try:
        logger.info("开始执行 browser CLI 登录检查: %s", config.llm_browser_cli_command)
        adapter.ensure_authenticated()
        logger.info("Browser CLI 认证成功，可继续执行模型调用")
        return 0
    except Exception as e:
        logger.error("Browser CLI 登录失败: %s", e)
        logger.error("请在终端手动执行登录命令后重试: %s", config.llm_browser_cli_login_command)
        return 1


def preflight_llm_browser_auth(config: Config) -> bool:
    """Run browser auth preflight before analysis flow."""
    if (config.llm_auth_mode or "").strip().lower() != "browser_cli":
        return True

    if not config.llm_browser_cli_auto_login:
        logger.info("LLM_BROWSER_CLI_AUTO_LOGIN=false，跳过启动前自动登录")
        return True

    result = run_llm_browser_login(config)
    if result == 0:
        return True

    if config.llm_browser_cli_allow_apikey_fallback:
        logger.warning("browser_cli 认证失败，已启用 API Key 回退，继续执行")
        return True

    logger.error("browser_cli 认证失败且未启用回退，程序退出")
    return False
...
def main() -> int:
    ...
    if args.llm_browser_login:
        logger.info("模式: browser CLI 登录")
        return run_llm_browser_login(config)

    if not preflight_llm_browser_auth(config):
        return 1
    ...
    try:
        ...
        if args.market_review:
            ...
            if (
                ...
                or config.llm_auth_mode == "browser_cli"
            ):
                ...
                logger.warning("未检测到可用 LLM 认证（API Key/browser_cli），将仅使用模板生成报告")
